...
\begin{abstract}
  ...
  mild (J$_{LW} \sim 2 - 10 \ J_{21}$(z)) Lyman-Werner (LW) background.
  We find that more than 20 very massive stars form with stellar
  masses greater than 1000 \msolarc. The most massive star has a stellar mass of 6128 \msolarc.
  ...
  maintained within the timescale (2 Myrs) of our simulations. As the massive stars end their lives
  as direct collapse black holes this will seed these embryonic haloes with a population of black
  holes with masses between 1000 \msolar and 10000 \msolarc,
  which may then go on to sink to the centre of the haloes, eventually coalescing to form larger
...
current theoretical models \citep[e.g.][]{Turk_2009, Clark_2008, Hirano_2014, Stacy_2016} the initial mass function should be top heavy with a characteristic mass close to 40 \msolarc. However, PopIII remnant black holes are expected to be born starving \citep{Whalen_2004, OShea_2005b, Milosavljevic_2009} and to reside in low
...
\caption{\textit{Left Panel:} The LW background rate imposed on Halo A for the re-simulation. \hb is not shown
  ...
  the halo was detected in the original (lower resolution) simulations. M$_{atom}$, orange line, is the atomic cooling threshold \citep{Fernandez_2014} and M$_{min,LW}$ is the threshold mass for halo collapse in the presence of an expected LW background at these redshifts \citep{Machacek_2001, OShea_2008}.
  }  \label{Fig:LWHistory}
...
was then smoothed on a physical\footnote{all distance units quoted are physical units unless explicitly stated otherwise} scale of 5 comoving Mpc, and regions of high
...
average ($\langle\delta\rangle \sim 0.09)$), and low ($\langle\delta\rangle \simeq -0.26)$)
 ...
 These sub-volumes were then referred to as the \rarepeak region and the \normal region  and the \void
...
halo with a mass of a few times $10^6$ \msolar due to the lack of LW background and thus can be classified as a mini-halo. Before analysing the star formation in detail we use results from \cloudy to determine when the negative feedback from stars eventually
...
\begin{minipage}{175mm}      \begin{center}
    ...
    \caption{
      ...
      In \hb on the other hand an average star has a mass of 43 Msolar and is shown by the dashed red line.
...
grows in mass up to its final mass of 6128 \msolarc within approximately 20 kyr, after which
...
\msolaryr (see right panel). What then drives the star to increase its mass is a merger
...
169 (M$_{DM} \sim 170$ \msolarc). Additionally, self-shielding of \molH from LW was included and the Jeans length was refined by at least 64 cells at all times.  To model star formation within the collapsing gas clouds a star formation prescription was employed, which additionally tracks accretion
