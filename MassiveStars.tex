...
\caption{\textit{Left Panel:} The percentage of the inner volume ionised in each halo
  ...
  dashed line. \ha is shown in blue, \hb in red. For both haloes the 1 pc volume srrounding the
  most massive star always becomes ionised but as we look to the 
  larger region (5 pc) the medium remains quite neutral. Note that for \ha the
  most massive star changes over time and this is reflected in the ionisation spikes seen.
  The first spike, where the volume becomes ionised quickly (at approximately 700 kyr) is at
  least partially due to the star moving to a lower density environment. 
  ...
  recombination rate of the gas. For \ha we see a similar trend to the left panel. The most massive star
  , at a time less than $\sim 1000$ kyr is quickly able to ionise its surroundings. However, at
  larger radii (and also at later times for \hac) the gas remains neutral. 
...
Modelling the ionized nebula associated with each star in this way does not account for the
...
in all of the surrounding haloes. In the re-simulation the adapative mesh refinement
...
We use a radius of 1 pc to estimate the ionisation state of the case close to the most massive star
...
for each halo, using a Monte Carlo approach. Note that for a constant combined ionizing luminosity,
...
ionising photons. The extent to which this would raise the total ionized volume beyond
our estimate depends sensitively on the local density distribution beyond each Str{\" o}mgren region.

Indeed, an alternative approach is to simply compare the ionizing photon luminosity of all stars
within the inner star-forming region with the total integrated recombination rate of the gas if
all hydrogen atoms were ionized; the latter sets the total emission rate of photons with
E$>$13.6eV needed to maintain ionization of the star-forming region in equilibrium. This can be
...
plasmas photoionized by Population III stars, $\rm{T}\sim 2\times 10^{4}$K
\citep{Osterbrock2006, Johnson2012}. 

These two measures of the strength of ionizing stellar feedback are compared in
...
with a 5 pc radius are shown as dashed lines. Note that the ionized volume (left panel)
...
ionized volume between $\sim 500$ and $\sim1200$ kyr when measured in a sphere of 1 pc, this is
matched in both cases by a similar rise in the fractional ionizing photon budget (right panel).
...
surrouning and this is at least partly because the star also moves into a lower density region
(very similar to what happens in \hbc). After 750 kyr the most massive star becomes embedded in
...
filaments in which recmonations are dominant and into which ionising photons cannot penetrate. \\
...
\indent In summary, ionizing stellar emission is unlikely to shut down star formation in
either halo prior to the onset of chemical and radiative emission from the supernovae
explosions expected to occur for some of the stars approximately 2 Myr  after their formation. In
this case of \ha this is particularly important. \ha contains a number of hyper-luminous PopIII stars
however, the much denser gas in the central region of \ha is robust against stellar feedback,
albeit with the latter growing in importance as the simulation progresses (see left panel
of Figure \ref{Fig:Ionisation}).
