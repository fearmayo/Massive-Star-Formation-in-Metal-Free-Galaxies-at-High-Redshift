...
\begin{abstract}
  ...
  mild ($J_{\rm LW} \sim 2 - 10 \ J_{21}$(z)) Lyman-Werner background.
  We find that more than 20 very massive stars form with stellar
  masses greater than 1000 \msolarc. The most massive star has a stellar mass of 6128 \msolarc. \jhw{All of the other numbers have less significant figures. Change to ``over 6000 \msolarc''?}
  ...
  Afterwards they may sink to the halo centre eventually coalescing to form larger
  intermediate mass black holes whose in-situ mergers will be detectable by LISA.  \jhw{I split the previous long sentence.}
\end{abstract}
...
current theoretical models \citep[e.g.][]{Turk_2009, Clark_2008, Hirano_2014, Stacy_2016} the initial mass function should be top heavy with a characteristic mass \change[close to 40 \msolarc]{of tens of solar masses}. However, PopIII remnant black holes are expected to \change[be born starving and to reside]{form} \jhw{I don't like using a starving baby analogy for a BH :)} in low
density environments \citep{Whalen_2004, Milosavljevic_2009} and are not expected to accrete substantially, at least not 
...
\indent SMSs provide an alternative path to forming a SMBH by giving the seed black hole a head-start compared to a black hole formed from a PopIII remnant. Under a SMS formation scenario the accretion rate onto the protostar must exceed a critical threshold thought to be around 0.04 \msolaryr \citep{Sakurai_2016}. When
this threshold accretion rate is reached and maintained the stellar radius inflates reducing \change[the effective]{its surface} temperature \delete{of the star} to approximately 5000 K
and making the star resemble a red giant star \citep{Omukai_2003, Hosokawa_2012, Hosokawa_2013, Woods_2017}. However, the SMS must continue to
...
\note{Reword some of the following text to avoid self-plagarism from the metal-enriched paper.}
...
universe in a 40 Mpc on the side box using the WMAP7 best fit cosmology \citep{Komatsu_2011}. Initial conditions
...
average ($\langle\delta\rangle \sim 0.09$), and low ($\langle\delta\rangle \simeq -0.26$)
 ...
 These sub-volumes were then referred to as the \rarepeakc, \normalc, and \void
 regions. The \rarepeak region has a comoving volume of 133.6 Mpc$^3$, the \normal and
  \void regions have comoving volumes of 220.5 Mpc$^3$. Each region was then re-simulated with
 ...
 of $2.9 \times 10^4$ \msolarc. For the re-simulations of these regions
 ...
 \msolarc. The \rarepeak regions contained 822 \change[galaxies]{haloes} with masses larger than $10^9$ \msolar at
 $z = 15$, the \normal region contained 758 such \change[galaxies]{haloes} at $z = 11.6$, while the \void region
 contained 458 such \change[galaxies]{haloes} at $z = 8$. \jhw{These counts are too high. Double-check.}
 ...
 \change[In order to re-simulate a halo from the original Renaissance datasets some of the parameters of the
 simulation  must be adjusted. In order to make the simulation time tractable, refinement
 must be focused around the target halo.]{In order to make the simulation time tractable, we restricted the mesh refinement to be focused around the target haloes.} In order to do this we first identified the Lagrangian
 volume of the target halo at the redshift at which it was found ($z = 15.6$ for \ha and $z = 15.0$
 for \hbc) and tagged each dark matter particle in the Lagrangian volume. \jhw{Only within the halo? Or did you use a multiple of the virial radius?} Upon restarting
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 With refinement targeted only on a single halo (and its progenitors) star formation in the surrounding galaxies is therefore neglected. In order to account for the radiation which would otherwise be emitted by these galaxies we extract the LW emission from the original simulations and create a table of LW values that this target halo is exposed to as a function of
 ...
 \note{Careful with repeating text from Regan \& Downes.}
...
\item The cell exceeds the Jeans Density \jhw{Reword. I know what you mean, but you should put it in terms of the Jeans length (or mass) and cell width.}
\item The flow around the cell is converging \delete{along each axis}
\item The cooling time of the cell is less than the freefall time
\item The cell is at \change[the]{a local} minimum of the gravitational potential
\end{enumerate}
Once the star is formed accretion onto the star is determined by evaluating the mass flux across a
\change[4 cell boundary surrounding]{sphere with a radius of 4 cells centered on} the star. Initially all stars are assumed to be stars with low surface
...
that timestep. Stars are merged when they come within 3 times the accretion radius of each other (i.e. 12 cell lengths). \jhw{Where does this factor of three come from? Krumholz+ (2004)? In my SS-runs, I've set it equal to the accretion radius.}\\
...
surface, to a PopIII star. \jhw{This is me nitpicking, but these SMSs are technically Pop III stars.  We might want to have a footnote somewhere in the paper, stating that when we say ``Pop III star,'' we mean a more typical massive star with hot surface temperatures.} This transitioning only occurs if the accretion rate onto the star either
...
main sequence \change[then the radiative feedback becomes significantly harder as the effective
temperature of the star increases dramatically. The radiation from a massive PopIII star is expected
to peak in the UV]{while its surface temperature dramatically increases up to $10^5$~K, causing its spectrum to harden and peak in the UV}. \\
\indent All stars in this simulation emit radiative feedback below the ionisation threshold of
hydrogen. \jhw{With ray tracing or an optically-thin field?} As stars in this simulation contract to the main sequence, when the accretion rate onto the
star drops, radiation above the hydrogen ionisation threshold is not initiated. \change[We found that the
density gradients produced by the massive PopIII stars proved too steep for the hydrodynamics solver
currently in place in \enzo]{The shocks generated near the star particle are too strong and unresolved for the PPM reconstruction scheme and HLLC Riemann solver to handle, where the steep gradients caused the solution to overshoot to negative densities and energies} (see \cite{Enzo_2014} for details). \change[]{We could have avoided this problem by utilizing a more diffusive solver at the expense of accuracy, but instead} \delete{As a result, for these simulations}
we \change[neglected]{neglect} the impact of ionising feedback \change[Instead we]{and} post-process the impact of
...
suppressed until after $z = 15$. Therefore, we can see here that the absence of the background LW radiation field was sufficient to allow PopIII star formation to take place. Dynamical heating by itself was not sufficient for this halo to 
...
halo with a mass of a few times $10^6$ \msolar and so can be classified as a mini-halo. \jhw{Give halo masses instead of approximate values.} Before analysing the star formation in detail we use results from \cloudy to determine when the negative feedback from stars eventually
...
\caption{We examine the radial profiles for both \ha (blue line) and \hb (red line). The four panels
  ...
  with the accretion rates that are seen onto the stars that subsequently form in each halo. \jhw{What is causing the flat lines (missing data?) in the infall rate panel? Is there net outflow at those radii, so you don't show those data?}}
...
\begin{minipage}{175mm}      \begin{center}
    ...
    \caption{
      ...
      In \hb on the other hand an average star has a mass of 43 \msolar and is shown by the dashed red line. \jhw{Should we show the median stellar mass, too, given that we show it in the right panel?  Also the reader can see how the distribution skewness changes over time when compared to the mean mass.}
...
results in a star of 173 \msolarc. \jhw{Is this physical, especially since it's 200~kyr after its creation?  We might want a statement giving a caveat or two.}\\
...
(18\%).  \jhw{Why did you choose these radii?  I'd think we would want to use the Jeans mass (i.e. the largest radius where $M_{\rm enclosed} > M_{\rm J}$).} In order for the formation of a truly supermassive star in \ha all of the stellar mass
...
hole orbits. \jhw{Does the gas dynamical friction depend on the densities? I'd think that given our higher resolution and densities, the dynamical friction should be stronger.}  The irregular stellar distributions found by \cite{Pfister_2019} correlates with what
...
may wander the galaxy for many dynamical times. \jhw{If their velocities are much smaller than the halo circular velocity, I think they'd wander in the inner regions instead of the whole galaxy.} The galactic centres in which our massive PopIII
...
GW signal detectable by LISA \citep{eLISA, Sesana_2016, Cornish_2020} emitting
...
169 (M$_{\rm DM} \sim 170$ \msolarc). Additionally, self-shielding of \molH from LW was included and the Jeans length was refined by at least 64 cells.  To model star formation within the collapsing gas clouds a star formation prescription was employed which additionally tracks accretion
